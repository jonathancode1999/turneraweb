...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        ...
        if ($a) {
            ...
            $stmtA->execute([':bid' => $bid, ':brid' => $branchId, ':id' => $id]);
            $a2 = $stmtA->fetch();
            if ($a2 && in_array($act, ['accept','cancel','complete','approve_reschedule','reject_reschedule'], true)) {
                // Timeline
          ...
          <td>
            <form method="post" style="display:flex;gap:8px;align-items:center;">
              ...
              <?php if ($r['status'] === 'PENDIENTE_APROBACION'): ?>
                <button class="btn primary" name="act" value="accept" type="submit">Aceptar</button>
                <button class="btn danger" name="act" value="cancel" type="submit" onclick="return confirm('¿Cancelar turno?');">Cancelar</button>
              <?php endif; ?>
