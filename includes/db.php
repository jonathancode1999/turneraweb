...
function ensure_multibranch_schema(PDO $pdo): void {
  ...
  if (!db_table_exists($pdo, 'message_templates')) {
    $pdo->exec("CREATE TABLE IF NOT EXISTS message_templates (
      ...
      PRIMARY KEY (business_id, event_key, channel)
    )");
  }
...
function migrate_if_needed(PDO $pdo): void {
    ...
    if ($version <= 0) {
        ...
        try {
            $pdo->exec($schema);
            $pdo->prepare("INSERT OR REPLACE INTO meta(key,value) VALUES('schema_version','11')")->execute();
            seed_demo_data($pdo);
            $pdo->commit();
        } catch (Throwable $e) {
            $pdo->rollBack();
    ...
    if ($version === 9) {
        ...
        try {
            ...
            $pdo->exec("UPDATE appointments SET status='PENDIENTE_APROBACION' WHERE status IN ('PENDIENTE_DE_PAGO')");
            ...
            $pdo->exec("UPDATE businesses SET public_base_url = COALESCE(public_base_url,'')");
	            ...
	            if (db_table_exists($pdo, 'businesses') && isset($haveBiz['gallery_title'])) {
	                ...
	                $pdo->exec("INSERT INTO businesses_new (
                        ...
                        slot_minutes,slot_capacity,cancel_notice_minutes,pay_deadline_minutes,customer_choose_barber,
	            ...
	            if (!db_table_exists($pdo, 'businesses')) {
	                ...
	                $pdo->exec("INSERT OR IGNORE INTO businesses(id,name,timezone,slot_minutes,pay_deadline_minutes) VALUES(1,'Turnera Demo','America/Argentina/Buenos_Aires',30,0)");
	            }
...
function seed_demo_data(PDO $pdo): void {
    ...
    // Business
    $pdo->prepare('INSERT INTO businesses (id, name, address, whatsapp_phone, timezone, slot_minutes, pay_deadline_minutes)
                   VALUES (1, :name, :addr, :wa, :tz, :slot, :deadline)')
        ->execute([
            ':name' => 'Turnera Demo',
            ...
            ':deadline' => (int)$cfg['pay_deadline_minutes'],
        ]);
    ...
    // Admin user
    $u = $cfg['seed_admin']['username'] ?? 'admin';
    $p = $cfg['seed_admin']['password'] ?? '1234';
    $hash = password_hash($p, PASSWORD_DEFAULT);
    $pdo->prepare('INSERT INTO users (business_id, username, password_hash, role)
                   VALUES (1, :u, :h, :r)')
        ->execute([':u' => $u, ':h' => $hash, ':r' => 'admin']);
    ...
    $stmt = $pdo->prepare('INSERT INTO services (business_id, name, description, duration_minutes, price_ars, deposit_ars, image_url, is_active)
                           VALUES (1, :n, :desc, :d, :p, :dep, :img, 1)');
...
function expire_pending_bookings(PDO $pdo): void {
    // v1 (sin seña): invalidamos enlaces de turnos ya pasados.
