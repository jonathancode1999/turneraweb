    ...
    try {
        if ($act === 'create') {
            ...
            try {
                ...
                $rel = upload_image_from_field('avatar_file', $uploadsDir, 'barber_' . $barberId . '_avatar', 4 * 1024 * 1024);
                if ($rel) {
                    $pdo->prepare("UPDATE barbers SET avatar_path=:av, updated_at=CURRENT_TIMESTAMP WHERE business_id=:bid AND branch_id=:brid AND id=:id")
                        ->execute([":av" => $rel, ":bid" => $bid, ":brid" => $branchId, ":id" => $barberId]);
                }
