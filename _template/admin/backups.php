...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  $ts = date('Ymd_His');
  if ($driver === 'sqlite') {
    // trigger sqlite file copy backup
    $src = __DIR__ . '/../data/app.sqlite';
    $dst = $dstDir . '/app_' . $ts . '.sqlite';
    @copy($src, $dst);
    flash('Backup creado: ' . basename($dst), 'ok');
  } else {
    // MySQL: dump to .sql (requires mysqldump available on server)
    ...
    // Use env var to avoid exposing password in process list when possible
...
if ($driver !== 'sqlite') {
  echo '<div class="muted" style="margin-top:8px">En MySQL se genera un <b>dump .sql</b> (requiere mysqldump).</div>';
}
echo '</form>';
...
if (!$files) {
  ...
  foreach ($files as $f) {
    $bn = basename($f);
    $dt = $bn;
    if (preg_match('/^app_(\d{8})_(\d{6})\.sqlite$/', $bn)) $dt = preg_replace('/^app_(\d{8})_(\d{6})\.sqlite$/', '$1 $2', $bn);
