...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if ($action==='create') {
    ...
    // Prevent duplicate usernames (friendly error instead of fatal PDO exception)
    $chk = $pdo->prepare('SELECT 1 FROM users WHERE business_id=:bid AND username=:u LIMIT 1');
    $chk->execute([':bid'=>$bizId, ':u'=>$u]);
    if ($chk->fetchColumn()) {
  ...
  if ($action==='delete') {
    ...
    $selfId = (int)($_SESSION['admin_user']['id'] ?? 0);
    if ($id === $selfId) {
      flash('No podés eliminar tu propio usuario.','err');
      redirect('users.php');
    }

    // Prevent deleting the last active admin
    $row = $pdo->prepare('SELECT role, is_active FROM users WHERE id=:id AND business_id=:bid');
    $row->execute([':id'=>$id, ':bid'=>$bizId]);
    $u = $row->fetch();
    if (!$u) {
      redirect('users.php');
    }
    if ((string)($u['role'] ?? '') === 'admin' && (int)($u['is_active'] ?? 0) === 1) {
      $cnt = $pdo->prepare('SELECT COUNT(*) FROM users WHERE business_id=:bid AND role="admin" AND is_active=1');
      $cnt->execute([':bid'=>$bizId]);
      $n = (int)$cnt->fetchColumn();
      if ($n <= 1) {
        flash('No podés eliminar el último admin activo.','err');
        redirect('users.php');
      }
    }

    // Remove branch access then delete user
    $pdo->prepare('DELETE FROM user_branch_access WHERE business_id=:bid AND user_id=:uid')
      ->execute([':bid'=>$bizId, ':uid'=>$id]);
    $pdo->prepare('DELETE FROM users WHERE id=:id AND business_id=:bid')
      ->execute([':id'=>$id, ':bid'=>$bizId]);
