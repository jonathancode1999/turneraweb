...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Anti-spam: protect cancel/reschedule endpoints
    try {
        spam_throttle_ip('manage', 40, 300);
        spam_throttle_token('manage', (string)$token, 20, 300);
    } catch (Throwable $e) {
        $error = $e->getMessage();
    }
    ...
    if ($action === 'reschedule') {
        ...
        try {
            ...
            if ($newBarberId === 0) {
				// Pick within the same branch and respecting service availability
...
<div class="container">
  <div class="card">
    ...
    <?php endif; ?>
	    ...
	    <a class="btn secondary" href="ics.php?token=<?php echo h(urlencode((string)$token)); ?>" style="<?php echo ($status === 'PENDIENTE_APROBACION') ? 'margin-top:12px;display:inline-block;' : ''; ?>">Agregar al calendario</a>
