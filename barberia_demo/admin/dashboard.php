            ...
            <tr>
              ...
              <td style="white-space:nowrap;display:flex;gap:8px;justify-content:flex-end;">
                <a class="btn" target="_blank" rel="noopener"
                   onclick="try{ this.closest('tr').remove(); }catch(e){}"
                   href="wa_action.php?act=reminder&id=<?= (int)$r['id'] ?>&csrf=<?= urlencode(csrf_token()) ?>&return=dashboard.php">Enviar</a>
                <a class="btn" style="background:#fff;border:1px solid #ccc;color:#444;"
                   onclick="try{ this.closest('tr').remove(); }catch(e){}"
                   href="wa_action.php?act=dismiss_reminder&id=<?= (int)$r['id'] ?>&csrf=<?= urlencode(csrf_token()) ?>&return=dashboard.php">×</a>
              </td>
            </tr>
          <?php endforeach; ?>
        </tbody>
