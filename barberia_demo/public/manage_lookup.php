...
// IMPORTANT: generate captcha AFTER processing POST, otherwise it would overwrite
// the expected answer in session and make valid submissions fail.
$captcha = null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  try {
    spam_throttle_ip('lookup', 20, 300);
  ...
  if ($error !== '') {
    // Don't process lookup if anti-spam failed.
  } else {
...
// Always show a captcha question (new one after each request)
